...
Move training scripts from flat `scripts/` into a proper Python package at `python/training/`. This separates Python from Rust, enables clean imports between modules, and prepares for V6.4 PyO3 bindings (which will live at `python/quant_gym/`).
...
7. CONVERGE     - freeze feature set for V6.3 gym observation space
```
...
## V6.3/V7 Feed-Forward

- **V6.3 Gym**: Ensemble agents become smart background agents the RL agent must compete against. Observation space = FullFeatures (frozen after experiment loop). ML cache features reusable for gym observations (Option A in 6.3 plan). Note: correlated ensemble agents may flood the order book -- may need reduced ensemble count or diversified threshold configs.
