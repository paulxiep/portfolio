...
- **Systems Programming (Rust):** `Tokio (Async Runtime)` · `Cargo Workspaces` · `Rayon (Parallel Iterators)` · *`PyO3 Python Bindings (planned)`* · `Tree-sitter (AST Parsing)` · `Ratatui/Crossterm (TUI)` · `Serde` · `Error Handling (anyhow/thiserror)` · `tracing` · `clap (CLI)` · `tokio-tungstenite` · `SmallVec`
